...
"""
    ...
    def place_hybrid_orders(self, symbol):
        ...
        try:
            ...
            if m15_data is not None and isinstance(m15_data, pd.DataFrame) and len(m15_data) > 0:
                ...
                # in_kill_zone = (6 <= current_hour <= 11) or (10 <= current_hour <= 20)
                in_kill_zone = (0 <= current_hour <= 20)  # 24/7 trading
    ...
    def find_ltf_entry_signals(self, symbol, htf_setup):
        """Find precise entry signals on lower timeframe - RELAXED VERSION"""
        try:
            ...
            print(f"🔍 DEBUG LTF: {symbol} {setup_type} at {htf_entry:.5f}, Current: {current_price:.5f}")
            
            # Calculate distance from HTF entry
            distance_percent = abs(current_price - htf_entry) / htf_entry * 100
            print(f"🔍 DEBUG LTF: Distance from HTF entry: {distance_percent:.3f}%")
            
            # RELAXED: Allow up to 0.3% distance from HTF entry (instead of 0.1%)
            price_threshold = 0.003  # 0.3%
            
            # LTF Entry Conditions for BUY
            if setup_type == 'BUY':
                # Relaxed condition: price near HTF level OR approaching from above
                if current_price <= htf_entry * (1 + price_threshold):
                    print(f"🔍 DEBUG LTF: {symbol} BUY - Price near HTF level")
                    
                    # Use tighter LTF-based SL for better RR
                    ...
                    if rr_ratio >= MIN_RR_RATIO:
                        print(f"🔍 DEBUG LTF: {symbol} BUY CONFIRMED - RR: {rr_ratio:.1f}")
                        return {
                            'symbol': symbol,
                            'type': 'BUY',
                            'entry': htf_entry,
                            ...
                            'rr_ratio': rr_ratio,
                            'htf_source': 'H4',
                            'ltf_confirmation': 'M5_Relaxed',
                            'reason': f"HTF OB + LTF Relaxed Confirmation (RR: {rr_ratio:.1f})"
                        }
            
            # LTF Entry Conditions for SELL
            elif setup_type == 'SELL':
                # Relaxed condition: price near HTF level OR approaching from below
                if current_price >= htf_entry * (1 - price_threshold):
                    print(f"🔍 DEBUG LTF: {symbol} SELL - Price near HTF level")
                    ...
                    if rr_ratio >= MIN_RR_RATIO:
                        print(f"🔍 DEBUG LTF: {symbol} SELL CONFIRMED - RR: {rr_ratio:.1f}")
                        return {
                            'symbol': symbol,
                            'type': 'SELL',
                            'entry': htf_entry,
            ...
            print(f"🔍 DEBUG LTF: {symbol} NO LTF confirmation - Distance: {distance_percent:.3f}%")
            return None
